...
For our database in order to keep our user secured we hash our users passwords before storing them into the database. We used werkzeug security helper function “generate_password_hash” to hash our function when the user is created. Our user is created in “http://localhost:5000/createUser” then when a user wants to be log in they need to be authenticated so they will need to use “http://localhost:5000/authenicateUser”. When a user wants to be authenticated they will need to provide their username and password. So our authenticated user api will get the username hash password from the database and check the users input password matches the hash password. We do this by using werkzeug security helper function “check_password_hash” this will the user if the password they provided was every hash and inserted in our database. 
